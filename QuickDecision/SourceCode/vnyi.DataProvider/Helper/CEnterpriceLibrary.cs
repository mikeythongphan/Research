...
namespace vnyi.DataProvider
{
    public partial class CEnterpriceLibrary
    {
            ...
            if (System.Configuration.ConfigurationManager.ConnectionStrings != null)
                if (System.Configuration.ConfigurationManager.ConnectionStrings.Count >= 0)
                    if (System.Configuration.ConfigurationManager.ConnectionStrings["Ezipos"] != null)
                        vnyi.DataProvider.CEnterpriceLibrary.sConnectionString = System.Configuration.ConfigurationManager.ConnectionStrings["Ezipos"].ConnectionString;
        }
        ...
        /// <param name="store">The store procedure.</param>
        /// <param name="Paras">The parameter, Output value to parameter has ParameterDirection attribute is Output.</param>
        public static IDataReader ExecuteReaderOut(string store, params SqlParameter[] Paras)
        {
            Database db = new SqlDatabase(sConnectionString);
            IDataReader Resul = null;
            using (DbConnection connect = db.CreateConnection())
            {
                if (connect.State == ConnectionState.Closed)
                    connect.Open();
                if (connect.State == ConnectionState.Open)
                {
                    try
                    {
                        DbCommand cmd = db.GetStoredProcCommand(store);
                        if (Paras != null)
                            foreach (SqlParameter ob in Paras)
                            {
                              
                                //In
                                if (ob.Direction == ParameterDirection.Input)
                                {
                                    db.AddInParameter(cmd, ob.ParameterName, ob.DbType, ob.Value);
                                }
                                //Out
                                else if (ob.Direction == ParameterDirection.Output)
                                {
                                    db.AddOutParameter(cmd, ob.ParameterName, ob.DbType, 255);
                                }
                                //Default
                                else
                                {
                        ...
                        //execute                        
                        Resul = db.ExecuteReader(cmd);
                        //Output
                        if (Paras != null)
                            foreach (SqlParameter ob in Paras)
                            {
                                if (ob.Direction == ParameterDirection.Output)
                                {
                                    ob.Value = db.GetParameterValue(cmd, ob.ParameterName.ToString().StartsWith("@") ? ob.ParameterName : ("@" + ob.ParameterName)).ToString();
                                }
                            }
                        connect.Close();
                    }
