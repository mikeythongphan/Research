...
{   
    public sealed class SqlHelper
    {
        ...
        public const string DatabaseConnectionKey = "databaseConection";
        private static string sConnectionString = string.Empty;
       

        /// <summary>
        /// Lấy câu kết nối với db
        /// </summary>
        private static void setConnectString()
        {
            if (System.Configuration.ConfigurationManager.ConnectionStrings != null)
                if (System.Configuration.ConfigurationManager.ConnectionStrings.Count >= 0)
                    if (System.Configuration.ConfigurationManager.ConnectionStrings["Ezipos"] != null)
                        sConnectionString = System.Configuration.ConfigurationManager.ConnectionStrings["Ezipos"].ConnectionString;
        }
        /// <summary>
        /// Gan coneckString
        /// </summary>
        ...
        public static SqlTransaction GetTransaction()
        {
            if (ConnectString == string.Empty)
                throw new Exception("Chuỗi kết nối không được null");
            try
            {
                SqlTransaction tran = null;
                SqlConnection conect = new SqlConnection(ConnectString);
                if (conect.State == ConnectionState.Closed)
                    conect.Open();
                tran = conect.BeginTransaction();
                return tran;
            }
            catch (Exception ex)
            {
                throw new Exception("Không mỏ được kết nối");
            }
        }
        public static void RollbackTransaction(SqlTransaction tran)
        {
            ...
            {
                SqlConnection connect = tran.Connection;
                tran.Rollback();
                CloseTransaction(connect);
        ...
        private static void CloseTransaction(SqlConnection conect)
        {
            if (conect == null)
                throw new Exception("Transaction is not null");
            else
        ...
        {
            if (ConnectString == null || ConnectString.Length == 0) throw new ArgumentNullException("connectionString");
        ...
        public static int ExecuteNonQuery(string spName, params object[] parameterValues)
        {
            ...
            if ((parameterValues != null) && (parameterValues.Length > 0))
            {
                ...
                SqlParameter[] commandParameters = SqlHelperParameterCache.GetSpParameterSet(ConnectString, spName);
        ...
        public static object ExecuteDataset(object typeDataSet, SqlConnection connection, CommandType commandType, string commandText, params SqlParameter[] commandParameters)
        {
            ...
            try
            {
                ...
                if (typeDataSet != null)
                {
                    try
                    {
                        dataAdatpter.Fill(((DataSet)typeDataSet));
    ...
    public sealed class SqlHelperParameterCache
    {
        ...
        /// <summary>
        /// Lấy danh sách parramter
        /// </summary>
        /// <param name="spName"></param>
        /// <returns></returns>
        public static SqlParameter[] GetSpParameterSet(string spName)
        {
            return GetSpParameterSet(SqlHelper.ConnectString, spName);
        }
        /// <summary>
